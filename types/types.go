...
func defaultExecutor(g *Cmd) Executor {
	return func(ctx context.Context, name string, debug bool, streamHandler StreamHandler, args ...string) (string, error) {
		...
		combineOutputBuffer := bytes.NewBufferString("")
		cmd := exec.CommandContext(ctx, name, args...)
		if streamHandler.Stdin != nil {
			cmd.Stdin = streamHandler.Stdin
		}

		cmd.Stdout = combineOutputBuffer
		if streamHandler.Stdout != nil {
			cmd.Stdout = io.MultiWriter(combineOutputBuffer, streamHandler.Stdout)
		}

		cmd.Stderr = combineOutputBuffer
		if streamHandler.Stderr != nil {
			cmd.Stderr = io.MultiWriter(combineOutputBuffer, streamHandler.Stderr)
		}

		err := cmd.Run()

		return combineOutputBuffer.String(), err
	}
}
